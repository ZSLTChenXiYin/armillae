...
/// Store wrapper that delegates all operations to an `InMemorySectionStore`
/// except `delete_original` which always fails (spec §12 test fixture for
/// abandon-cleanup failure recovery).
pub struct FailOnDeleteOriginalStore {
    inner: crate::memory::InMemorySectionStore,
}

impl FailOnDeleteOriginalStore {
    pub fn new() -> Self {
        Self {
            inner: crate::memory::InMemorySectionStore::new(),
        }
...
impl Default for FailOnDeleteOriginalStore {
    fn default() -> Self {
        Self::new()
    }
}

impl crate::store::SectionStore for FailOnDeleteOriginalStore {
    fn save_state(
        &self,
        state: &crate::store::SectionState,
    ) -> Result<(), crate::store::StoreError> {
    ...
    fn save_compressed(
        &self,
        entry: &crate::store::SectionCompressedEntry,
    ) -> Result<crate::store::CompressedRef, crate::store::StoreError> {
        self.inner.save_compressed(entry)
    }
    fn load_compressed(
        &self,
        session_id: &str,
    ...
    fn delete_original(
        ...
        Err(crate::store::StoreError::Backend {
            message: "FailOnDeleteOriginalStore always fails delete_original".to_owned(),
        })
    }
}
